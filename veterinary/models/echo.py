...
class Echo(models.Model):
    ...
    def action_echo_sent(self):
        ...
        try:
            template_id = ir_model_data.get_object_reference('veterinary', 'email_template_echo')[1]
        except ValueError:
            template_id = False
        # try:
...
class MailComposeMessage(models.TransientModel):
    ...
    def send_mail(self, auto_commit=False):
        if self._context.get('default_model') == 'veterinary.eco' and self._context.get('default_res_id') and self._context.get('mark_so_as_sent'):
